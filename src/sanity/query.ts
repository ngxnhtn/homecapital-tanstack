...
export const propertyCardFragments = q
  .fragmentForType<'property'>()
  .project((sub) => ({
    id: sub.field('_id'),
    featureImage: sub
      .field('featureImage.asset')
      .transform((img) => urlForImage(img!).url()),
...
export const agencyQuery = q.star
  ...
  .project((sub) => ({
    ...
    logo: sub
      ...
      .deref()
      .transform((img) =>
        urlForImage(img!).width(100).height(100).format('png').url(),
      ),
  }))
  .notNull()
  ...
  q.star
    ...
    .project((sub) => ({
      ...
      gallery: sub.field('gallery[]').project((i) => ({
        url: i
          .field('asset')
          .transform((img) => (img ? urlForImage(img).url() : '')),
      })),
    }))
